...
import { faUser, faEnvelope, faPhone , faFileUpload } from "@fortawesome/free-solid-svg-icons"
...
export default function Partner() {
    ...
    const submitHandler = async (e: React.FormEvent) => {
        e.preventDefault();
        if (name.length === 0 || email.length === 0 || address.length === 0 || phoneNumber.length === 0)
                        ...
                        <form method="POST" className="register-form" id="register-form" onSubmit={submitHandler}>
                            <div className="user-form-group">
                                ...
                                <input type="text" name="name" id="name"
                                    placeholder="Partner Name" className="user-input"
                                    value={name} onChange={(e) => setname(e.target.value)} />
                            </div>
                            <div className="user-form-group">
                                <FontAwesomeIcon icon={faEnvelope} style={{ margin: '20px 0px' }} />
                                <input type="email" name="email" id="email"
                                    placeholder="Partner Email" className="user-input"
                                    value={email} onChange={(e) => setemail(e.target.value)} />
                            </div>
                            <div className="user-form-group">
                                <FontAwesomeIcon icon={faPhone} style={{ margin: '20px 0px' }} />
                                <input type="number" name="phoneNumber" id="phoneNumber"
                                    placeholder="Partner phone Number" className="user-input"
                                    value={phoneNumber} onChange={(e) => setphoneNumber(e.target.value)} />
                            </div>
                            <div className="user-form-group">
                                <FontAwesomeIcon icon={faEnvelope} style={{ margin: '20px 0px' }} />
                                <textarea name="address" id="address"
                                    placeholder="Partner address" className="user-input"
                                    value={address} onChange={(e) => setaddress(e.target.value)} />
                            </div>
                           
                            <div className="user-form-group">
                                <FontAwesomeIcon icon={faFileUpload}  />
                                <button className='UploadLoagoBtn'  type='button'> Upload Logo</button>
                            ...
                            <div className="user-form-button">
                                <input type="submit" name="user-add" id="signup" className="add-user" value="Create Partner" />
                            </div>
                        </form>
                    </div>
