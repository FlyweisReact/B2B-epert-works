...
const LMSVideos: React.FC = (props: any) => {
  ...
  const [coursePercentageCompleted,setcoursePercentageCompleted]=useState<number>(0)
  ...
  const playVideo = (currentVideoElement: CurrentVideoElement) => {
    ...
    const playVideoWithSrc = () => {
      ...
      //if(authState.access_token || id[1]<0)
      //if(authState.access_token )
      try{
        //if(authState.access_token || id[1]<2)
  ...
  const playAnother = (offset: number) => {
    if(currentVideo.currentIndex+offset > 0 && currentVideo.currentIndex+offset < currentVideoElements.length) {
  ...
  //   });
  // }, []);
  const notify = () => toast.info("Please visit the pricing page to subscribe it");
// useEffect(()=>{
...
const courseId:any=localStorage.getItem("selectedCourseId")
  ...
  if (authState.access_token) {
    ...
    const videoListPromise = restClient.getCall(
      //ProdConfiguration.PROD_COURSE_URL + "/courses/" + courseId,
      //below is temp for percentage
    ...
    videoListPromise.then((videoData) => {
      ...
      //mapDispatch("SELECT_PLAY_VIDEO", response);
      renderVideo(response)
    });
  } else {
    // props.history.push("/pricing");
    // mapDispatch("PAGE", "Login");
    const videoListPromise = restClient.getCall(
    ...
    videoListPromise.then((videoData) => {
      ...
      //mapDispatch("SELECT_PLAY_VIDEO", response);
      // props.history.push(`/course/${coursename}`);
    });
  }
}
const renderVideo=(videoData:any)=>{
  setVideoData(videoData)
  try { if (videoData.result) {
    let counter: number = 0;
    ...
    videoData.result.sections.forEach((section: any,index:number) => {
      ...
      if (section.videoLinks) {
        ...
        section.videoLinks.forEach((videoLink: any) => {
          ...
          console.log("videoLink?.type",videoLink?.type,videoLink?.type!=='doc')
          currentVideoElements.push(videoElement);
          courseSection.push(
            <div className="courseSection-sub-box" key={"" + ++courseCounter}>
              <span
                style={{
                  position: "fixed",
                  visibility: "hidden",
              ...
              {section?.type!=='doc'?
                ...
                >
                  <div>
                    <input type="checkbox" disabled checked={section.completed} style={{}} />
                  </div>
        ...
        subSections.push(
          <Accordion className={authState.access_token||index<=2?'':classes.blurImage}>
            <AccordionSummary
            ...
            <AccordionDetails>
              <Typography>
                <div className="courseSection">{courseSection}</div>
              </Typography>
            </AccordionDetails>
          </Accordion>
        );
        ...
        <p style={{fontSize : '2rem'}}>Python Beginner</p>
          <div className="player_section">
            <div
              ...
              id="dynamic_video"
            ></div>
            <span className="right-arrow" onClick={()=>playAnother(1)}><FontAwesomeIcon icon={faArrowAltCircleRight}/></span>
            <div className="courseInfo"></div>
          </div>

         {Object.keys(videoData).length>0&& <CoursesDashBoard subSections={subSections} videoData={videoData}/>}

        </div>
        <div className="sidebar">
          <h2 className="courseContent"> Course Content <select style={{width : '200px' , outline : 'none'}}>
            <option>Beginner</option>
            <option>Intermediate</option>
            <option></option>
            </select>  </h2>
          <ProgressBar timervalue={coursePercentageCompleted} maxValue={10} />
          <span style={{fontSize:'14px'}}><strong>{authState.access_token&&`${(coursePercentageCompleted*100).toFixed(0)} % Completed`}</strong> </span>
