...
const LMSVideos: React.FC = (props: any) => {
  ...
  const { name : courseName   , level : courseLevel} = useParams()
  const [ documentData , setDocumentData ] = useState([])

  const fetchDocument  = async () => {
    try{
        // const { data } = await axios.get(`https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/ressouce/name/${courseName}-${courseLevel}`)
        const { data } = await axios.get(`https://52pv9t2fl3.execute-api.ap-south-1.amazonaws.com/dev/api/v1/ressouce/name/Python-beginner`)
    ...
    <div className="mainDivVideos no-scroll">
      ...
      <div className="flex-container">
        <div className="rightBar">
          <p style={{ fontSize: "2rem"  text-transform: capitalize; }}> {courseName} </p>
...
<Accordion>
  ...
  <AccordionDetails>
    <Typography> */}
      {/* <div className="courseSection">Resources</div> */}
      {/* <div
            className="resources"
            onClick={(e) => {
              // e.stopPropagation();
