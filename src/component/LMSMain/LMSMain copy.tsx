            ...
            <div className="container">
              <div className="row">
                <div className="col-lg-12">
                  <h2 className="pageTitle">Courses - <span className="beta-syle">Beta Version</span></h2>
                </div>
              </div>
            </div>
          </section>
              ...
              <div className="filter-items">
                {/* {allCourses
                  .filter((thing: any, i: any, arr: any) => {
                    return (
                      arr.indexOf(arr.find((t: any) => t.id === thing.id)) === i
                    );
                  })
                  .map( */}
                <div
          ...
          <div className="filtered-courses">
            ...
            <div className="video_heading">
              {/* <div className="course-heading">
                <div>
                  {authState && <h2>Let's Start learning, {authState.user}</h2>}
                </div>
                <div>
                  {" "}
              ...
              {authState && <h2 style={{ marginBottom: '20px' }}>Let's Start learning, {authState.name}</h2>}
              <h2 style={{ marginBottom: "20px" }}> Our Top Courses</h2>
              <p style={{ marginBottom: "20px !important" }}>
                Upskill Your Career with Expert Technologies.{" "}
              </p>
            ...
            {/* //////////////// */}
            <div>
              {type === 'FREE COURSES' && !authState ? <div>
                <h2>Sign up for free to start Learning.</h2>

                <Link to="/signup"> <button className="create-new-acc">Create Account</button></Link> </div> : null}
            </div>
            <div className="container-fluid proj-bottom">
                          ...
                          <p onClick={() => clickHandler(course)}>
                            ...
                            <div className="courses-down-content">
                              ...
                              <div className="rating-left">
                                <FontAwesomeIcon icon={faClock} /> {`${course.hours || '12'} hours`}
                              </div>
                              {/* <div className="rating-left">
                                <Rating value={course.rating} name="rating" readOnly /><span>{course.rating} </span><span>{`( ${course.reviews || 'No'} reviews)`}</span>
                                <h4> {Math.floor(course.price) === 0 ? 'free' :<>
                                {course.currencies&&<> <span className="cart-override-price"> {course.currencies[0].actualAmount}  </span><span>{course.currencies[0].amount}</span></>
                               } </>}</h4>
                              </div> */}
              ...
              <div className="courses">
                <div className="heading-level">
                  {/* if courses filtred, show all courses */}
                  {type.length === 0 && <h2 className="level">Intermediate</h2>}
                </div>
                <div
                  className="course-align"
                >
