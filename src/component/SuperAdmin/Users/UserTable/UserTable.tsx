...
export default function UserTable(props:any) {
  ...
  const cols:Column<Data1>[]=[
    ...
    { title: 'Role', field: 'userRole', initialEditValue: 'initial edit value' },
    // {
    //   title: "Custom Add",
  ...
  let userData: any = useSelector((state: RootState) => state.userEntitlementReducer.userData);
  ...
  let TeamData: any = useSelector((state: RootState) => state.superAdminreducer.TeamData);
    ...
    return (
        <div >
            <MaterialTable title="Added Admins" data={userData||[]} columns={columns}
            isLoading={loading}
            options={
                {
                    search:false
